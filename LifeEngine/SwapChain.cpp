...
void SwapChain::init(Device& device, DeviceContext& deviceContext, Texture& backBuffer, Window window)
{
	if (window.m_hWnd == nullptr) {
		WARNING("ERROR: SwapChain::init : Error in data from params [CHECK FOR HWND)\n ");
		exit(1);
	}
	...
	if (FAILED(hr))
	{
		WARNING("ERROR: SwapChain::init : Error in D3D11CreateDeviceAndSwapChain\n ");
		exit(1);
	}
	hr = m_swapChain->GetBuffer(0, __uuidof(ID3D11Texture2D), (LPVOID*)&backBuffer.m_texture);
	if (FAILED(hr))
	{
		WARNING("ERROR: SwapChain::init : Error in m_SwapChain->GetBuffer\n ");
		exit(1);
	}
}
